...
// ── Option 2: Launch History Intelligence ────────────────────────
function LaunchHistoryIntelligenceTab() {
  const timeline = [
    { period: '1980-1989', success: 7, failed: 2 },
    { period: '1990-1999', success: 12, failed: 3 },
    { period: '2000-2009', success: 19, failed: 4 },
    { period: '2010-2019', success: 28, failed: 3 },
    { period: '2020-2026', success: 27, failed: 3 },
  ]
  const vehicles = [
    { name: 'PSLV', missions: 62, successRate: 92 },
    { name: 'GSLV Mk II', missions: 28, successRate: 82 },
    { name: 'LVM3/GSLV Mk III', missions: 18, successRate: 89 },
  ]
  const seasonal = [
    { season: 'Pre-Monsoon', successRate: 91 },
    { season: 'Monsoon', successRate: 78 },
    { season: 'Post-Monsoon', successRate: 93 },
    { season: 'Winter', successRate: 90 },
  ]
  const shap = [
    { feature: 'precipitation_mm', impact: 0.31 },
    { feature: 'wind_speed', impact: 0.24 },
    { feature: 'cloud_cover', impact: 0.19 },
    { feature: 'humidity_pct', impact: 0.14 },
    { feature: 'is_monsoon', impact: 0.12 },
  ]
  ...
  return (
    <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}} className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-5 space-y-6">
        <GlassPanel className="p-6">
          <h3 className="font-display font-bold text-lg text-white mb-4">🌧️ Success Rate by Season</h3>
          <div className="space-y-3">
            {seasonal.map((s) => (
              <div key={s.season}>
                <div className="flex justify-between text-xs text-slate-400 mb-1">
                ...
                <div className="w-full h-2 rounded-full bg-slate-800">
                  <div className={`h-2 rounded-full ${s.season === 'Monsoon' ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${s.successRate}%` }} />
                </div>
              </div>
            ))}
          </div>
...
// ── Option 3: Chandrayaan Mission Archive ───────────────────────
  ...
  const missions = [
    {
      ...
      year: 2008,
      status: 'Completed',
      highlight: 'Confirmed evidence of water/hydroxyl signatures on the lunar surface.',
    },
    {
      title: 'Chandrayaan-2',
      year: 2019,
      status: 'Orbiter operational',
      highlight: 'High-value orbiter science continues with terrain and composition observations.',
    },
    {
      title: 'Chandrayaan-3',
      year: 2023,
      status: 'Lander mission completed',
      highlight: 'Soft landing near the lunar south polar region; surface ops concluded in Sep 2023.',
    },
  ]
...
export default function IsroMissionCenter() {
  const tabs = [
    ...
    { value: 'launch-intel', label: '📈 Launch Intelligence' },
    { value: 'archive',      label: '🌙 Mission Archive' },
    ...
    <div className="min-h-screen bg-[#0a0e17] text-slate-200 font-body">
      <div className="mx-auto max-w-[1400px] px-6 lg:px-8 py-8">
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'launch-intel' && <LaunchHistoryIntelligenceTab key="launch-intel" />}
          {activeTab === 'archive' && <ChandrayaanArchiveTab key="archive" />}
